...
export default function Settings() {
  ...
  const { user } = useAuth()
  const { preference: themePreference, resolvedTheme, setPreference: setThemePreference, accent, accentOptions, setAccent } = useTheme()
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: '#0E1B1F', color: '#F7FBFA' }}>
      ...
      <main className="mx-auto max-w-2xl px-4 pb-10 pt-5 sm:px-6 sm:pt-10">
        ...
        <section className="mt-8" aria-labelledby="appearance-heading"><div className="flex items-center justify-between px-1"><div><h2 id="appearance-heading" className="text-sm font-black uppercase tracking-[.18em]" style={{ color: '#91A7AD' }}>Appearance</h2><p className="mt-2 text-xs" style={{ color: '#6F858B' }}>Choose how DataKwest looks on this device.</p></div><span className="text-xs font-black" style={{ color: '#8BC6B5' }}>{resolvedTheme === 'dark' ? 'Dark' : 'Light'} now</span></div><div className="mt-3 grid grid-cols-3 gap-2 rounded-2xl border p-2" style={{ borderColor: '#40565D', background: '#14252A' }}>{[['system', 'System'], ['light', 'Light'], ['dark', 'Dark']].map(([value, label]) => <button key={value} type="button" onClick={() => setThemePreference(value)} aria-pressed={themePreference === value} className="min-h-11 rounded-xl px-2 text-xs font-black" style={{ background: themePreference === value ? '#8BC6B5' : 'transparent', color: themePreference === value ? '#0E1B1F' : '#91A7AD' }}>{label}</button>)}</div></section>

        <section className="mt-5 rounded-2xl border p-4" style={{ borderColor: '#40565D', background: '#14252A' }} aria-labelledby="accent-heading"><div className="flex items-center justify-between gap-3"><div><h3 id="accent-heading" className="text-sm font-black">Accent colour</h3><p className="mt-1 text-xs" style={{ color: '#91A7AD' }}>Choose a high-contrast highlight for links, focus, and progress.</p></div><span className="text-xs font-black" style={{ color: 'var(--dk-accent)' }}>{accentOptions[accent]?.label || 'Ocean'}</span></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{Object.entries(accentOptions).map(([key, option]) => <button key={key} type="button" onClick={() => setAccent(key)} aria-pressed={accent === key} className="flex min-h-12 items-center gap-2 rounded-xl border px-3 text-left text-xs font-black" style={{ borderColor: accent === key ? option.primary : '#40565D', background: accent === key ? option.soft : '#0E1B1F', color: accent === key ? option.primary : '#91A7AD' }}><span className="h-4 w-4 shrink-0 rounded-full" style={{ background: option.primary }} aria-hidden="true" />{option.label}</button>)}</div></section>
