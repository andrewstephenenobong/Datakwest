...
function getErrorStatus(error) {
  return error?.status || error?.context?.status || error?.cause?.status || null
}

function getErrorCode(error) {
  return error?.code || error?.context?.code || error?.cause?.code || ''
}

async function describeOnboardingError(error) {
  ...
  const permissionFailure = status === 401 || status === 403 || code === '42501' || /permission denied|row-level security|not authenticated/i.test(message)
  if (permissionFailure) {
    if (status === 401 || /not authenticated/i.test(message)) return 'Your session has expired. Please sign in again, then we can finish building your roadmap.'
    return 'Your learning plan is ready, but we are reconnecting your learning space. Your answers are safe. Please try again in a moment.'
  }
  if (status === 429 || /rate limit|usage limit|too many requests/i.test(message)) {
    return 'The roadmap service is temporarily rate-limited. Your answers are still here. Please wait a moment and try again.'
  }
  if (!navigator.onLine || /failed to fetch|network|timeout|timed out|load failed|fetch failed/i.test(message)) {
    return 'Your connection interrupted roadmap generation. Your answers are still here. Reconnect, then try again.'
  }
  return 'We could not finish building your roadmap right now. Your answers are still here. Please try again.'
}
...
export default function Onboarding() {
  ...
  return (
    <div className="onboarding-page min-h-screen">
      <header className="flex items-center justify-between px-6 py-5 sm:px-10"><div className="flex items-center gap-3"><img src="/datakwest_logo_lockup.png" alt="DataKwest logo" className="h-12 w-52 object-contain object-left" /><div><p className="onboarding-header-wordmark text-sm font-black tracking-tight" style={{ color: '#0A2342' }}>DATAKWEST</p><p className="onboarding-header-tagline text-[10px] font-semibold uppercase tracking-[0.18em]" style={{ color: '#8391A7' }}>Career operating system</p></div></div><div className="onboarding-header-meta hidden sm:flex items-center gap-3 text-xs font-semibold" style={{ color: '#6B7A99' }}><span className="h-2 w-2 rounded-full" style={{ background: '#37A169' }} />Saved automatically <span className="mx-1" style={{ color: '#D6DEE9' }}>|</span><span>{stepIndex + 1} / {steps.length}</span></div></header>
