...
const navigationSounds = await readFile('src/lib/navigationSounds.js', 'utf8')
...
test('branded recovery states cover 404 and application errors', () => {
  ...
  assert.match(recoveryState, /Let’s give that page another try\./)
  assert.match(recoveryState, /Home base is ready\./)
  assert.match(recoveryState, /localStorage\.setItem\('datakwest-owl-sound'/)
  assert.match(recoveryState, /aria-pressed=\{soundStyle === value\}/)
...
test('owl loading and profile settings keep the authenticated shell consistent', () => {
  ...
  assert.match(settingsPage, /Navigation sounds/)
  assert.match(settingsPage, /Keep me signed in/)
  assert.match(navigationSounds, /datakwest-navigation-sounds/)
