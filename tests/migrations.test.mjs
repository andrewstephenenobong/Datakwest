...
const mlEvaluationScript = await readFile('ml/evaluate_mastery_model.py', 'utf8')
...
const skillGraphMigration = await readFile('backend/supabase/migrations/0029_skill_graph_foundation.sql', 'utf8')
const learnerEvidenceMigration = await readFile('backend/supabase/migrations/0030_learner_evidence_foundation.sql', 'utf8')
const learningIntelligenceRpcMigration = await readFile('backend/supabase/migrations/0031_learning_intelligence_rpc.sql', 'utf8')
...
test('universal skill graph foundation is versioned, provenance-aware, and publish-gated', () => {
  assert.match(skillGraphMigration, /create table if not exists public\.skill_domains/)
  assert.match(skillGraphMigration, /create table if not exists public\.skill_graph_versions/)
  assert.match(skillGraphMigration, /create table if not exists public\.skill_graph_nodes/)
  assert.match(skillGraphMigration, /create table if not exists public\.skill_graph_edges/)
  assert.match(skillGraphMigration, /create table if not exists public\.skill_sources/)
  assert.match(skillGraphMigration, /create table if not exists public\.skill_graph_node_sources/)
  assert.match(skillGraphMigration, /create table if not exists public\.learning_object_versions/)
  assert.match(skillGraphMigration, /alter table public\.skill_graph_versions enable row level security/)
  assert.match(skillGraphMigration, /status = 'published'/)
  assert.match(skillGraphMigration, /review_status = 'approved'/)
  assert.match(skillGraphMigration, /age_band text not null/)
})

test('learner evidence is immutable, private, and server-computed', () => {
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_skill_enrolments/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_evidence/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.evidence_attempts/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.evidence_artifacts/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_node_mastery/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_skill_state/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_preferences/)
  assert.match(learnerEvidenceMigration, /create table if not exists public\.learner_interaction_events/)
  assert.match(learnerEvidenceMigration, /alter table public\.learner_evidence enable row level security/)
  assert.match(learnerEvidenceMigration, /Users can view own evidence/)
  assert.match(learnerEvidenceMigration, /model_version text not null/)
  assert.doesNotMatch(learnerEvidenceMigration, /create policy .*insert.*learner_evidence/i)
})

test('learning intelligence RPCs keep evidence and mastery server-authoritative', () => {
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.create_skill_enrolment/)
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.start_learning_evidence/)
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.submit_learning_evidence/)
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.get_learner_skill_state/)
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.update_learner_preferences/)
  assert.match(learningIntelligenceRpcMigration, /create or replace function public\.record_learner_interaction/)
  assert.match(learningIntelligenceRpcMigration, /security definer/)
  assert.match(learningIntelligenceRpcMigration, /auth\.uid\(\)/)
  assert.match(learningIntelligenceRpcMigration, /revoke all on function public\.submit_learning_evidence/)
  assert.match(learningIntelligenceRpcMigration, /grant execute on function public\.submit_learning_evidence.*authenticated/s)
  assert.match(learningIntelligenceRpcMigration, /mastery_updated.*false/)
})

test('initial skill catalogue seeds the ten launch paths and a publishable starter graph', () => {
  assert.match(initialSkillCatalogMigration, /insert into public\.career_paths/)
  assert.match(initialSkillCatalogMigration, /frontend-development/)
  assert.match(initialSkillCatalogMigration, /cybersecurity/)
  assert.match(initialSkillCatalogMigration, /business-productivity/)
  assert.match(initialSkillCatalogMigration, /insert into public\.skills/)
  assert.match(initialSkillCatalogMigration, /insert into public\.skill_graph_versions/)
  assert.match(initialSkillCatalogMigration, /status = 'published'/)
...
test('ML training workflow is real-data-only, grouped, and shadow-only', () => {
  ...
  assert.match(mlEvaluationScript, /no_evaluation_rows/)
  assert.match(mlEvaluationScript, /beats_baseline/)
  assert.match(mlEvaluationScript, /certifiable/)
  assert.match(mlEvaluationScript, /serving_mode.*shadow/)
...
test('learning intelligence client uses protected RPCs for learner state and evidence', () => {
  assert.match(learningIntelligenceClient, /async function callLearningRpc\(name, args = \{\}\)/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('create_skill_enrolment'/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('start_learning_evidence'/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('submit_learning_evidence'/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('get_learner_skill_state'/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('update_learner_preferences'/)
  assert.match(learningIntelligenceClient, /callLearningRpc\('record_learner_interaction'/)
  assert.doesNotMatch(learningIntelligenceClient, /from\(['"]learner_(evidence|node_mastery|skill_state)/)
  assert.doesNotMatch(learningIntelligenceClient, /\.insert\(/)
  assert.doesNotMatch(learningIntelligenceClient, /\.update\(/)
})
